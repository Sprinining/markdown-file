...
```java
switch (msgBaseSetFreqRange.getFreqRangeIndex()) {
    case "FCC":
        region = 1;
        break;
    case "ETSI":
    ...
    case "China2":
        ...
        break;
    default:
        return new Message<>(Constants.MSG_BASE_SET_FREQ_RANGE, null, (byte) 0, "工作频段不对");
}
```

```java
public class MsgBaseSetFreqRange {
    // 频段索引 FCC ETSI China1 China2
...
switch (region.getData()) {
    ...
    case "RG_EU3":
        ...
        break;
    default:
        regionStr = region.getData();
}
// 返回的可能是China1 China2 FCC ETSI或原始字符串
allParameters.setRegion(regionStr);
```
...
```json
{
  ...
  "buzzerEnable": true,
  "rtCode": 0,
  "rtMsg": "附加数据为空0 gpi1 gpi2 gpi3 gpi4 "
}
...
- 附加数据设置byteCount为奇数时，实际返回（奇数-1）个字节
- 附加数据：bank：保留区0，EPC区1，TID区2，USER区3
